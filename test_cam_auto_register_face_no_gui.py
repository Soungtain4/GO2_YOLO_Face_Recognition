"""
Face Registration Tool (No GUI Version)
- Capture face from webcam without display
- Auto-capture after countdown
- Save to registered_faces/
- Update visitors_info.json
- Camera: Auto-detection enabled
"""
...
def get_available_cameras(max_cameras=10):
    """
    Scan for available cameras indices
    
    Args:
        max_cameras: Maximum index to check
...
def register_new_face(camera_index=0, auto_mode=True):
    """
    ...
    print("\n" + "="*60)
    print("FACE REGISTRATION (No GUI Version)")
    print("="*60)
    print()
...
def main():
    """Main function"""
    print("\n" + "="*60)
    print("FACE REGISTRATION SYSTEM (No GUI)")
    print("="*60)
    ...
    print(f"Auto-selected camera index: {selected_camera_index}")
    ...
    while True:
        ...
        if choice == '1':
            try:
                register_new_face(camera_index=selected_camera_index, auto_mode=True)
            ...
            try:
                register_new_face(camera_index=selected_camera_index, auto_mode=False)
            except KeyboardInterrupt:
                print("\n\nRegistration interrupted by user")
            except Exception as e:
                print(f"\nError: {e}")
