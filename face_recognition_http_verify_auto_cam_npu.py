"""
YOLO + InceptionResnetV1 Face Recognition System (NPU Version + HTTP Mode + Verify)
...
- Logic: "Verify Before Send" (User confirms y/n before sending)
"""
...
class YOLOFaceRecognitionSystemHTTPVerify:
    def __init__(self,
    ...
    def run_recognition(self, camera_index=0):
        """
        Main loop for HTTP Verify Mode (NPU)
        """
        ...
        print("\n" + "="*60)
        print("Face Recognition Started (HTTP Verify Mode - NPU)")
        print("="*60)
        print(" - Detects faces continuously")
        print(f" - Asks for confirmation before sending to {self.server_url}")
                    ...
                    for idx, (box, score) in enumerate(zip(boxes, scores)):
                        ...
                        try:
                            ...
                            if result:
                                ...
                                # Logic: Ask for confirmation before sending
                                print(f"\n[MATCH] Recognized: {name}")
                                print(f" -> Destination: {destination}")

                                # Ask user confirmation
                                user_input = input(f"Send destination '{destination}' via HTTP? (y/n): ").strip().lower()
                                ...
                                if user_input == 'y':
                                    ...
                                    print(" -> Cancelled by user. Continuing detection...")
                                    time.sleep(1)
...
def main():
    ...
    # Initialize System
    system = YOLOFaceRecognitionSystemHTTPVerify(server_url=server_url)
